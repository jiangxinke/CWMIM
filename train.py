...
from Trainers import StdTrainer
...
if __name__ == '__main__':
    ...
    # initialize
    model = GCN(args.seq_len, args.pre_len, slope_set.locs, args).to(args.device)
    ...
    if args.valid:
        ...
        trainer = StdTrainer(model, loss, optimizer, train_loader, valid_loader, test_loader, scaler, args)
    else:
        test_set = Subset(slope_set, range(int(slope_set.len * args.train), slope_set.len))
        test_loader = DataLoader(test_set, batch_size=args.test_batch_size, shuffle=True)

        trainer = StdTrainer(model, loss, optimizer, train_loader, None, test_loader, scaler, args)
